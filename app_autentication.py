...
# from client_llm import GmailMCPClient
# from client2 import GmailMCPClient
...
# Titulo con estado de autenticación
col1, col2 = st.columns([3, 1])
with col1:
    st.title("📧 Gmail Assistant con MCP")
    st.markdown("Asistente inteligente para gestionar tu Gmail usando Gemini")
...
with st.sidebar:
    ...
    if client.is_authenticated():
        st.success("✅ Token válido encontrado")
    ...
    if "error" in info:
        ...
        st.warning("Verifica que el servidor esté ejecutándose y el token sea válido")
    else:
...
st.divider()
st.caption("Gmail Assistant powered by MCP + Gemini | 🔒 Autenticación Bearer Token")